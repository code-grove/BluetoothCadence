...
public class BluetoothManager: NSObject {
    ...
    private func parseCscMeasurement(_ data: Data) {
        ...
        if wheelRevolutionDataPresent && data.count >= index + 6 {
            let wheelRevolutions = UInt32(littleEndian: data[index..<index+4].withUnsafeBytes { $0.load(as: UInt32.self) })
            index += 4
            let wheelEventTime = UInt16(littleEndian: data[index..<index+2].withUnsafeBytes { $0.load(as: UInt16.self) })
            index += 2
    ...
    internal func calculateCadence(newCrankRevolutions: UInt16, newCrankEventTime: UInt16) {
        ...
        let revDiff = (UInt32(newCrankRevolutions) - UInt32(lastCrankRevolutions)) & 0xFFFF
